...
class Open9xInterface : public EEPROMInterface
{
  ...
  protected:
    ...
    template <class T>
    bool saveModelVariantNew(unsigned int index, ModelData &model, uint32_t variant);

    template <class T>
    bool loadGeneral(GeneralSettings &settings);

    template <class T>
    bool saveGeneral(GeneralSettings &settings, uint32_t variant, uint32_t version);
